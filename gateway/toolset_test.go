...
func TestToolSetRoundTrip(t *testing.T) {
	...
	if len(ts.Endpoints) != len(origTools) {
		t.Fatalf("endpoints = %d, want %d", len(ts.Endpoints), len(origTools))
	}
	...
	if len(tools) != len(origTools) {
		t.Fatalf("round-trip tools = %d, want %d", len(tools), len(origTools))
	}
	if len(endpoints) != len(origEndpoints) {
		t.Fatalf("round-trip endpoints = %d, want %d", len(endpoints), len(origEndpoints))
	}

	// Verify each tool matches
	for i, tool := range tools {
		orig := origTools[i]
		if tool.Function.Name != orig.Function.Name {
			t.Errorf("tool[%d] name = %q, want %q", i, tool.Function.Name, orig.Function.Name)
		}
		if tool.Function.Description != orig.Function.Description {
			t.Errorf("tool[%d] desc = %q, want %q", i, tool.Function.Description, orig.Function.Description)
		}
	}
