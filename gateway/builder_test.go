...
func TestBuildToolsCount(t *testing.T) {
	...
	// petstore.json has 4 operations + 1 set_auth = 5 tools, 4 endpoints
	if len(tools) != 5 {
...
func TestSanitizeToolNameTruncationUnique(t *testing.T) {
	// Two long names sharing a 58-char prefix must produce different results
	prefix := "target__post_bitable_v1_apps_app_token_tables_table_id_recor"
	a := sanitizeToolName(prefix + "ds_batch_create")
	b := sanitizeToolName(prefix + "ds_batch_update")
	if a == b {
		t.Errorf("truncated names should differ: both = %q", a)
	}
	if len(a) > 64 || len(b) > 64 {
		t.Errorf("names exceed 64 chars: a=%d b=%d", len(a), len(b))
	}
}
