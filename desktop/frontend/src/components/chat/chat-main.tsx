...
export function ChatMain({ conversationId, onConversationCreated }: ChatMainProps) {
    ...
    EventsOn("chat-event", (event: { type: string; data: Record<string, unknown> }) => {
      switch (event.type) {
        ...
        case "done": {
          ...
          // Skip reload when error occurred — error message is frontend-only
          // and would be overwritten by backend state
          const hadError = errorRef.current;
          errorRef.current = false;
          if (finalConvId && !hadError) {
        ...
        case "error": {
          ...
          setMessages((prev) => [
            ...prev,
            { id: nextId(), role: "assistant", content: `Error: ${errMsg}`, timestamp: new Date(), ephemeral: true },
          ]);
          setLoading(false);
          scrollToBottom();
          break;
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          { id: nextId(), role: "assistant", content: `${t("chat.error")}: ${errMsg}`, timestamp: new Date(), ephemeral: true },
        ]);
        setLoading(false);
      }
    },
  ...
  const handleRetryMessage = useCallback(
    async (frontIdx: number, backendIdx: number) => {
      if (!conversationId) return;
      setMessages((prev) => prev.slice(0, frontIdx));
      setLoading(true);
      setUsage(null);
      try {
        await RegenerateFrom(conversationId, backendIdx);
      } catch (err) {
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-2 sm:px-4 py-3">
        <div className="space-y-3 max-w-none md:max-w-3xl mx-auto">
          ...
          {messages.map((msg, idx) => {
            const isEphemeral = msg.ephemeral;
            // Backend index = count of non-ephemeral messages before this one
            const backendIdx = isEphemeral ? -1 : messages.slice(0, idx).filter((m) => !m.ephemeral).length;
            return (
              <ChatMessage
                key={msg.id}
                message={msg}
                isLast={idx === messages.length - 1 && msg.role === "assistant"}
                onEdit={!isEphemeral && msg.role === "user" ? (newContent) => handleEditMessage(backendIdx, newContent) : undefined}
                onDelete={!isEphemeral ? () => handleDeleteMessage(backendIdx) : undefined}
                onRetry={!isEphemeral && msg.role === "assistant" && idx === messages.length - 1 ? () => handleRetryMessage(idx, backendIdx) : undefined}
              />
            );
          })}
